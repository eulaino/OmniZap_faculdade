...
function CommandModalComponent({ visible, onClose }: CommandModalProps) {
  ...
  const podeEnviar =
    textComando.trim().length > 0 && horaValida(horaTexto) && !createMutation.isPending;
        ...
        <View className="w-full overflow-hidden rounded-[28px] border border-slate-200 bg-slate-50 shadow-sm">
          ...
          <View className="gap-4 px-5 py-5">
            <View className="rounded-2xl border border-slate-200 bg-white p-4">
              <Text
                style={{ fontFamily: 'Inter_400Regular' }}
                className="mb-2 text-[11px] uppercase tracking-[0.12em] text-slate-500">
                Comando
              </Text>
            ...
            <View className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              {horaInvalida ? (
                <Text className="mt-2 text-xs font-semibold text-red-500">Horário inválido.</Text>
              ) : null}
            </View>
